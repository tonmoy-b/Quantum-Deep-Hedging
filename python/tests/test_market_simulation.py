...
def default_heston_model_params():
    return dict(n_paths=50000, S0=100.0, v0=0.04, mu=0.05,
                kappa=2.0, theta=0.04, sigma_v=0.3, rho=-0.6, T=1.0, dt=0.01)
...
def test_expected_price(default_heston_model_params):
    ...
    actual = S[:, -1].mean()
    assert torch.isclose(actual, expected, rtol=0.02), \
